...
// Get Agentic Flow intelligence metrics (SONA / HNSW / ReasoningBank + q-learning
// routing). Prefers agentic-flow's native .agentic-flow/intelligence.json; falls
// back to claude-flow's .claude-flow/metrics/learning.json so the line is never blank.
function getAgenticFlowStats() {
  let patterns = 0, memories = 0, routes = 0, trajectories = 0;
  let lr = 10, eps = 10, active = false, source = 'none';

  // 1. agentic-flow native intelligence file (after `agentic-flow hooks pretrain`)
  const intelPath = path.join(process.cwd(), '.agentic-flow', 'intelligence.json');
  if (fs.existsSync(intelPath)) {
    try {
      const d = JSON.parse(fs.readFileSync(intelPath, 'utf-8'));
      patterns = Array.isArray(d.patterns) ? d.patterns.length : 0;
      memories = Array.isArray(d.memories) ? d.memories.length : 0;
      routes = d.metrics?.totalRoutes || 0;
      trajectories = Array.isArray(d.trajectories) ? d.trajectories.length : 0;
      if (typeof d.config?.learningRate === 'number') lr = Math.round(d.config.learningRate * 100);
      if (typeof d.config?.epsilon === 'number') eps = Math.round(d.config.epsilon * 100);
      active = true; source = 'agentic-flow';
    } catch (e) { /* ignore */ }
  }

  // 2. Fallback: claude-flow learning metrics
  if (source === 'none') {
    const learnPath = path.join(process.cwd(), '.claude-flow', 'metrics', 'learning.json');
    if (fs.existsSync(learnPath)) {
      try {
        const d = JSON.parse(fs.readFileSync(learnPath, 'utf-8'));
        patterns = (d.patterns?.shortTerm || 0) + (d.patterns?.longTerm || 0);
        memories = d.patterns?.longTerm || 0;
        routes = d.routing?.decisions || 0;
        source = 'claude-flow';
      } catch (e) { /* ignore */ }
    }
  }

  // 3. Is the agentic-flow runtime actually running? (cheap process check)
  try {
    const n = parseInt(execSync('ps aux 2>/dev/null | grep -c "[a]gentic-flow"', { encoding: 'utf-8', timeout: 1000 }).trim(), 10);
    if (n > 0) active = true;
  } catch (e) { /* ignore */ }

  return { patterns, memories, routes, trajectories, lr, eps, active, source };
}
...
function generateStatusline() {
  ...
  const agenticFlow = getAgenticFlowStats();
  const lines = [];
  ...
  // Line 5: Agentic Flow intelligence — SONA patterns/memory/routes + q-learning routing
  const afDot = agenticFlow.active ? `${c.brightGreen}🐝${c.reset}` : `${c.dim}⚡${c.reset}`;
  const afColor = (agenticFlow.patterns > 0 || agenticFlow.routes > 0) ? c.brightGreen : c.dim;
  lines.push(
    `${c.brightPurple}🧠 Agentic Flow${c.reset} ${afDot}  ` +
    `${afColor}◆${agenticFlow.patterns}${c.reset} ${c.dim}patterns${c.reset}  ` +
    `${afColor}⬡${agenticFlow.memories}${c.reset} ${c.dim}mem${c.reset}  ` +
    `${afColor}↝${agenticFlow.routes}${c.reset} ${c.dim}routes${c.reset}    ` +
    `${c.brightYellow}🎯 q-learning${c.reset} ${c.cyan}lr:${agenticFlow.lr}%${c.reset} ${c.cyan}ε${agenticFlow.eps}%${c.reset}`
  );
