/**
 * Magic Link Authentication Service
 * TASK-PORTAL-011: Parent Portal Magic Link Authentication
 *
 * Provides passwordless authentication for parents via email magic links.
 * Uses JWT tokens with short expiry for security.
 *
 * Flow:
 * 1. generateMagicLink() - Creates JWT token, sends email
 * 2. verifyMagicLink() - Validates token, returns parent info
 * 3. createParentSession() - Creates longer-lived session token
 */

import {
  Injectable,
  Logger,
  UnauthorizedException,
  InternalServerErrorException,
} from '@nestjs/common';
import { JwtService } from '@nestjs/jwt';
import { ConfigService } from '@nestjs/config';
import { PrismaService } from '../../../database/prisma/prisma.service';
import { MailgunService } from '../../../integrations/mailgun/mailgun.service';

/** Magic link token payload */
interface MagicLinkPayload {
  sub: string; // Parent ID
  email: string;
  tenantId: string;
  type: 'magic_link';
}

/** Session token payload */
interface SessionPayload {
  sub: string; // Parent ID
  email: string;
  tenantId: string;
  type: 'parent_session';
}

/** Parent info returned after verification */
export interface VerifiedParent {
  id: string;
  email: string;
  firstName: string;
  lastName: string;
  phone: string | null;
  tenantId: string;
}

/** Configuration for magic link */
interface MagicLinkConfig {
  expiresInMinutes: number;
  sessionExpiresInHours: number;
  portalBaseUrl: string;
}

@Injectable()
export class MagicLinkService {
  private readonly logger = new Logger(MagicLinkService.name);
  private readonly config: MagicLinkConfig;

  constructor(
    private readonly jwtService: JwtService,
    private readonly prisma: PrismaService,
    private readonly mailgunService: MailgunService,
    private readonly configService: ConfigService,
  ) {
    this.config = {
      expiresInMinutes: parseInt(
        this.configService.get<string>('MAGIC_LINK_EXPIRY_MINUTES') || '15',
        10,
      ),
      sessionExpiresInHours: parseInt(
        this.configService.get<string>('PARENT_SESSION_EXPIRY_HOURS') || '24',
        10,
      ),
      portalBaseUrl:
        this.configService.get<string>('PARENT_PORTAL_URL') ||
        this.configService.get<string>('FRONTEND_URL') ||
        'http://localhost:3000',
    };
  }

  /**
   * Generate a magic link for parent login and send via email.
   * Returns success even if email doesn't exist (security: don't reveal valid emails).
   *
   * @param email - Parent's email address
   * @returns true if email was sent (or would have been sent)
   */
  async generateMagicLink(email: string): Promise<boolean> {
    this.logger.debug(`Magic link requested for: ${email}`);

    // Find parent by email (case-insensitive)
    const parent = await this.prisma.parent.findFirst({
      where: {
        email: {
          equals: email,
          mode: 'insensitive',
        },
        isActive: true,
        deletedAt: null,
      },
    });

    if (!parent) {
      // Don't reveal that email doesn't exist - log and return success
      this.logger.warn(`Magic link requested for non-existent email: ${email}`);
      return true;
    }

    if (!parent.email) {
      this.logger.warn(`Parent ${parent.id} has no email address`);
      return true;
    }

    // Generate JWT token with short expiry
    const payload: MagicLinkPayload = {
      sub: parent.id,
      email: parent.email,
      tenantId: parent.tenantId,
      type: 'magic_link',
    };

    const token = this.jwtService.sign(payload, {
      expiresIn: `${this.config.expiresInMinutes}m`,
    });

    // Build magic link URL
    const magicLinkUrl = `${this.config.portalBaseUrl}/parent/verify?token=${encodeURIComponent(token)}`;

    // Send email via Mailgun
    try {
      await this.sendMagicLinkEmail(
        parent.email,
        parent.firstName,
        magicLinkUrl,
      );
      this.logger.log(`Magic link sent to parent: ${parent.id}`);
      return true;
    } catch (error) {
      this.logger.error(
        `Failed to send magic link email: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw new InternalServerErrorException(
        'Failed to send magic link. Please try again later.',
      );
    }
  }

  /**
   * Send the magic link email using Mailgun.
   */
  private async sendMagicLinkEmail(
    email: string,
    firstName: string,
    magicLinkUrl: string,
  ): Promise<void> {
    const subject = 'Your CrecheBooks Parent Portal Login Link';

    const text = `
Hi ${firstName},

You requested to sign in to the CrecheBooks Parent Portal.

Click the link below to sign in:
${magicLinkUrl}

This link will expire in ${this.config.expiresInMinutes} minutes.

If you didn't request this link, you can safely ignore this email.

Best regards,
The CrecheBooks Team
    `.trim();

    const html = `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
</head>
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="text-align: center; margin-bottom: 30px;">
    <h1 style="color: #1a1a1a; margin: 0;">CrecheBooks</h1>
    <p style="color: #666; margin: 5px 0 0 0;">Parent Portal</p>
  </div>

  <p>Hi ${firstName},</p>

  <p>You requested to sign in to the CrecheBooks Parent Portal.</p>

  <div style="text-align: center; margin: 30px 0;">
    <a href="${magicLinkUrl}"
       style="display: inline-block; background-color: #0070f3; color: white; padding: 14px 28px; text-decoration: none; border-radius: 8px; font-weight: 600;">
      Sign In to Parent Portal
    </a>
  </div>

  <p style="color: #666; font-size: 14px;">
    This link will expire in ${this.config.expiresInMinutes} minutes.
  </p>

  <p style="color: #666; font-size: 14px;">
    If you didn't request this link, you can safely ignore this email.
  </p>

  <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">

  <p style="color: #999; font-size: 12px; text-align: center;">
    Best regards,<br>The CrecheBooks Team
  </p>
</body>
</html>
    `.trim();

    await this.mailgunService.sendEmail({
      to: email,
      subject,
      text,
      html,
      tags: ['parent-portal', 'magic-link'],
    });
  }

  /**
   * Verify a magic link token and return parent information.
   *
   * @param token - JWT token from magic link
   * @returns Verified parent information
   * @throws UnauthorizedException if token is invalid or expired
   */
  async verifyMagicLink(token: string): Promise<VerifiedParent> {
    this.logger.debug('Verifying magic link token');

    let payload: MagicLinkPayload;

    try {
      payload = this.jwtService.verify<MagicLinkPayload>(token);
    } catch (error) {
      if (error instanceof Error && error.name === 'TokenExpiredError') {
        this.logger.warn('Magic link token expired');
        throw new UnauthorizedException({
          message: 'Magic link has expired. Please request a new one.',
          code: 'TOKEN_EXPIRED',
        });
      }
      this.logger.warn(
        `Invalid magic link token: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw new UnauthorizedException('Invalid magic link');
    }

    // Validate token type
    if (payload.type !== 'magic_link') {
      this.logger.warn('Invalid token type for magic link verification');
      throw new UnauthorizedException('Invalid magic link');
    }

    // Verify parent still exists and is active
    const parent = await this.prisma.parent.findUnique({
      where: { id: payload.sub },
    });

    if (!parent || !parent.isActive || parent.deletedAt) {
      this.logger.warn(`Parent account not found or inactive: ${payload.sub}`);
      throw new UnauthorizedException('Account not found or inactive');
    }

    this.logger.log(`Magic link verified for parent: ${parent.id}`);

    return {
      id: parent.id,
      email: parent.email || '',
      firstName: parent.firstName,
      lastName: parent.lastName,
      phone: parent.phone,
      tenantId: parent.tenantId,
    };
  }

  /**
   * Create a session token for the authenticated parent.
   * This token has a longer expiry for convenience.
   *
   * @param parentId - Parent's UUID
   * @param email - Parent's email
   * @param tenantId - Tenant UUID
   * @returns Session token and expiry info
   */
  createParentSession(
    parentId: string,
    email: string,
    tenantId: string,
  ): { token: string; expiresIn: number } {
    const payload: SessionPayload = {
      sub: parentId,
      email,
      tenantId,
      type: 'parent_session',
    };

    const expiresIn = this.config.sessionExpiresInHours * 3600; // Convert to seconds

    const token = this.jwtService.sign(payload, {
      expiresIn: `${this.config.sessionExpiresInHours}h`,
    });

    this.logger.log(`Session created for parent: ${parentId}`);

    return { token, expiresIn };
  }

  /**
   * Verify a session token and return parent information.
   *
   * @param token - Session token
   * @returns Verified parent information or null
   */
  async verifySessionToken(token: string): Promise<VerifiedParent | null> {
    try {
      const payload = this.jwtService.verify<SessionPayload>(token);

      if (payload.type !== 'parent_session') {
        return null;
      }

      const parent = await this.prisma.parent.findUnique({
        where: { id: payload.sub },
        include: {
          _count: {
            select: { children: true },
          },
        },
      });

      if (!parent || !parent.isActive || parent.deletedAt) {
        return null;
      }

      return {
        id: parent.id,
        email: parent.email || '',
        firstName: parent.firstName,
        lastName: parent.lastName,
        phone: parent.phone,
        tenantId: parent.tenantId,
      };
    } catch {
      return null;
    }
  }
}
