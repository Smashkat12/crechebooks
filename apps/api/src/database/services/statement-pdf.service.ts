...
export class StatementPdfService {
   ...
   * TASK-BILL-043: Enhanced with bank details from tenant configuration
   */
  private renderPaymentInstructions(
    ...
    // TASK-BILL-043: Bank details section
    ...
    if (hasBankDetails) {
      doc
        .font('Helvetica-Bold')
        .fillColor(PDF_CONSTANTS.TEXT_PRIMARY)
        .text('Banking Details:', PDF_CONSTANTS.MARGIN_LEFT, y);
      y += 14;

      doc.font('Helvetica').fillColor(PDF_CONSTANTS.TEXT_SECONDARY);
      doc.text(`Bank: ${tenant.bankName}`, PDF_CONSTANTS.MARGIN_LEFT, y);
      y += 12;
      doc.text(
      ...
      if (tenant.bankAccountType) {
        doc.text(
          `Account Type: ${tenant.bankAccountType}`,
          PDF_CONSTANTS.MARGIN_LEFT,
          y,
        );
        y += 12;
      ...
      if (tenant.bankSwiftCode) {
        doc.text(
          `SWIFT Code: ${tenant.bankSwiftCode}`,
          PDF_CONSTANTS.MARGIN_LEFT,
          y,
        );
        y += 12;
    ...
    doc.fillColor(PDF_CONSTANTS.TEXT_SECONDARY);
    doc.text(
