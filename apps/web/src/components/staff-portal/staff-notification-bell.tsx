...
} from '@/hooks/staff-portal/use-staff-notifications';
...
export function StaffNotificationBell() {
  ...
  const { count, refresh: refreshCount } = useStaffUnreadCount();
  const { notifications, refresh: refreshList } = useStaffNotificationList();
  const markAsRead = useStaffMarkAsRead();
  const markAllAsRead = useStaffMarkAllAsRead();
